/* Luana Toledo — interações */
(() => {
  'use strict';

  const WHATSAPP = '5512991581917';
  ...
  let indice = 0;
  let nomeAtual = '';
  // Índice a partir do qual as fotos são do estado original (antes da reforma).
  let inicioAntes = Infinity;
  ...
  const render = () => {
    ...
    const ehAntes = indice >= inicioAntes;
    lupaImg.src = fotos[indice];
    lupaImg.alt = `${nomeAtual}${ehAntes ? ', antes da reforma' : ''}, foto ${indice + 1} de ${fotos.length}`;
    lupaContador.textContent = `${indice + 1} / ${fotos.length}`;
    lupa.classList.toggle('lupa--antes', ehAntes);
  ...
  const abrirLupa = (obra) => {
    ...
    const qtdAntes = Number(obra.dataset.antes) || 0;
    inicioAntes = qtdAntes ? fotos.length - qtdAntes : Infinity;
    indice = 0;
  ...
  form.addEventListener('submit', (e) => {
    ...
    const texto = [
      `Olá, Luana! Meu nome é ${form.nome.value.trim()}.`,
